...
// Get the absolute path to the `/locale` directory containing message
// translation files.
const messagesPath = join(root, 'locale', 'messages');
...
const locales = flow(
  ...
  // defined there.
  process.env.LOCALES
    ? intersectionBy(toLower, process.env.LOCALES.split(' '))
    : identity,
...
// Parse the current languages from the current locales by removing the country
// codes from the locale identifiers. React intl locale-data polyfill modules
// are defined only by language, not country as well.
const languages = flow(map((locale) => locale.split('-')[0]), uniq)(locales);

// React intl bundles `en` language data by default, so we never want to include
// id separately.
...
const createContextReplacementPlugin = (contextRegex, moduleRegex, ...args) => {
  const logger = instrumentContextModule(
    ...
    (module, context) =>
      console.log(
        `   ${
        relative(root, join(context, module))
      } ${
        chalk.green('✔')
      } `
    ),
...
export default () => (config) => {
  ...
  return flow(
    ...
    // replacement plugin to replace the default regex with our own. This allows
    // us to place stricter requirements on which files are matched within a
    // context.
    //
    ...
    // smaller set of locales can be defined with the `LOCALES` env variables if
    // not all available translations should be included in the current build.
    //
    ...
    plugin(createContextReplacementPlugin(
      ...
      // Define a regex that tests modules returned in the above context against
      // the current list of available locales.
    ...
    plugin(createContextReplacementPlugin(
      ...
      // Define a regex that tests modules returned in the above context against
      // the current list of available langauges.
