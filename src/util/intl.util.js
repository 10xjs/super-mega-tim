 ...
 *
 * Memoized to return the same Promise instance for any given language string.
 * This prevents multiple requests to the same async chunks on the client.
 *
  ...
  (language) => new Promise((resolve, reject) => {
    ...
    try {
      req = require(/* adana-no-instrument */ `${
        ...
        // ContextReplacementPlugin in the `locale` webpack config partial to
        // match only the current included set of locales for the build.
 ...
 * action is necessary.
 *
 * Memoized to return the same Promise instance for any given locale string.
 * This prevents multiple requests to the same async chunks on the client.
 *
 * @param   {String} locale - A locale string, e.x. `en-US`.
 * @returns {Promise} - A Promise.
 ...
 * @returns {Promise} - A Promise.
 */
export const loadLocaleData = (locale = 'en_US') => Promise.all([
  ...
  (locale) => new Promise((resolve, reject) => {
    ...
    try {
      req = require(/* adana-no-instrument */ `${
        ...
        // chunk.
          : ''
        // Create a require context for `../../locale/messages/**/*.yml` files.
        // The files within this context are further filtred by the
        // ContextReplacementPlugin in the `locale` webpack config partial to
        // match only the current included set of locales for the build.
        }../../locale/messages/${locale}.yml`
      );
    } catch (e) {
      return reject(e);
    }
